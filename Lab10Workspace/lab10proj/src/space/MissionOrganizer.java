 ...
 * Then expensive missions (that cost >= $1 trillion) are presented in an order determined by the subclass.
 * TreeMissionOrganizer iterates expensive missions by increasing cost; missions with identical cost are
 * iterated alphabetically by destination. ListMissionOrganize iterates expensive missions in insertion order.
 */


abstract public class MissionOrganizer implements Iterable<Mission>
{
	private final static float			ONE_TRILLION = 1.0e12f;
	
	private ArrayList<Mission>			cheapMissions;
	private Collection<Mission>			expensiveMissions;		// a TreeSet<Mission> or an ArrayList<Mission>
	
	
	abstract Collection<Mission> getExpensionMissionsCollection();
	
	
	MissionOrganizer()
	{
		cheapMissions = new ArrayList<>();
		expensiveMissions = getExpensionMissionsCollection();
	}
	...
	public static void main(String[] args)
	{
		...
		MissionOrganizer organizer = new TreeMissionOrganizer();
...
/*
Expected output using TreeMissionOrganizer:
...
Expected output using ListMissionOrganizer:
